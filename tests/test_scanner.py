...
class TestScanTorrentFile(SetupTeardown):
  ...
  def test_return_torrent_filepath(self, red_api, ops_api):
    copy_and_mkdir(get_torrent_path("red_source"), "/tmp/input/red_source.torrent")
  ...
  def test_call_injector_if_provided(self, red_api, ops_api):
  ...
  def test_call_injector_if_torrent_is_duplicate(self, red_api, ops_api):
    injector_mock = MagicMock()
    injector_mock.inject_torrent = MagicMock()
...
class TestScanTorrentDirectory(SetupTeardown):
  ...
  def test_list_generated_torrents(self, capsys, red_api, ops_api):
    copy_and_mkdir(get_torrent_path("red_source"), "/tmp/input/red_source.torrent")
    ...
    assert f"{Fore.LIGHTGREEN_EX}Found with source 'OPS' and generated as '/tmp/output/OPS/foo [OPS].torrent'.{Fore.RESET}" in captured.out
    assert f"{Fore.LIGHTGREEN_EX}Generated for cross-seeding{Fore.RESET}: 1" in captured.out

  def test_list_undecodable_torrents(self, capsys, red_api, ops_api):
  ...
  def test_list_unknown_tracker_torrents(self, capsys, red_api, ops_api):
  ...
  def test_list_already_existing_torrents(self, capsys, red_api, ops_api):
    copy_and_mkdir(get_torrent_path("red_source"), "/tmp/input/red_source.torrent")
  ...
  def test_consider_input_torrents_as_already_existing(self, capsys, red_api, ops_api):
    copy_and_mkdir(get_torrent_path("red_source"), "/tmp/input/red_source.torrent")
    ...
    assert f"{Fore.LIGHTYELLOW_EX}Torrent already exists in input directory at /tmp/input/red_source.torrent{Fore.RESET}" in captured.out
    assert f"{Fore.LIGHTYELLOW_EX}Already exists{Fore.RESET}: 2" in captured.out

  def test_consider_output_torrents_as_already_existing(self, capsys, red_api, ops_api):
    copy_and_mkdir(get_torrent_path("red_source"), "/tmp/input/red_source.torrent")
    copy_and_mkdir(get_torrent_path("ops_source"), "/tmp/output/ops_source.torrent")
  ...
  def test_call_injector_on_duplicate(self, capsys, red_api, ops_api):
    injector_mock = MagicMock()
    injector_mock.inject_torrent = MagicMock()
    ...
    assert f"{Fore.LIGHTYELLOW_EX}Torrent was previously generated but was injected into your torrent client.{Fore.RESET}" in captured.out
    assert f"{Fore.LIGHTYELLOW_EX}Already exists{Fore.RESET}: 1" in captured.out
    injector_mock.inject_torrent.assert_called_once_with(
      "/tmp/input/red_source.torrent", "/tmp/output/ops_source.torrent", "OPS"
    )

  def test_list_torrents_that_already_exist_in_client(self, capsys, red_api, ops_api):
    injector_mock = MagicMock()
    injector_mock.inject_torrent = MagicMock()
  ...
  def test_list_not_found_torrents(self, capsys, red_api, ops_api):
    copy_and_mkdir(get_torrent_path("red_source"), "/tmp/input/red_source.torrent")
  ...
  def test_list_unknown_error_torrents(self, capsys, red_api, ops_api):
    copy_and_mkdir(get_torrent_path("red_source"), "/tmp/input/red_source.torrent")
  ...
  def test_report_progress_for_mix_of_torrents(self, capsys, red_api, ops_api):
    ...
    assert f"{Fore.LIGHTGREEN_EX}Found with source 'RED' and generated as '/tmp/output/RED/foo [RED].torrent'.{Fore.RESET}" in captured.out
    assert f"{Fore.LIGHTGREEN_EX}Generated for cross-seeding{Fore.RESET}: 1" in captured.out
    assert f"{Fore.LIGHTBLACK_EX}Torrent not from OPS or RED based on source or announce URL{Fore.RESET}" in captured.out
    assert f"{Fore.LIGHTBLACK_EX}Skipped{Fore.RESET}: 1" in captured.out
    assert f"{Fore.RED}Error decoding torrent file{Fore.RESET}" in captured.out
  ...
  def test_ignore_non_torrent_files(self, capsys, red_api, ops_api):
