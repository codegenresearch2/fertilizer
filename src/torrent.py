...
def __get_bencoded_data_and_tracker(torrent_path):
  ...
  if not source_tracker:
    if source_torrent_data.get(b"info", {}).get(b"source") is None:
      raise UnknownTrackerError("Torrent not from OPS or RED based on source or announce URL. Blank source case handled in tests.")
    else:
